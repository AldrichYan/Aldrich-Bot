...
exports.commands =
{
	...
	tour: function(arg, by, room)
	{
		...
		if (!hasTourStarted)
		{
			...
			switch (arglist[0])
			{
				...
				case "tallcup":
				case "tall":
					tourObject = tourJSON["gen8yaomingcup"];
					break;
	default:
	...
	insult: function(arg, by, room)
	{
		...
		if (toID(arglist[0]).includes("AldrichYan") || toID(arglist[0]).includes("ald") || toID(arglist[0]).includes("4ld") || toID(arglist[0]).includes("aldrich") || toID(arglist[0]).includes("yan") || toID(arglist[0]).includes(toID(config.nick)) || /[^\u0000-\u007F]/g.test(arglist[0]))
		{
	...
	{
		this.say(room, "/addhtmlbox <a href='https://spo.ink/ansena'>AldrichYan's blog</a>");
	},
